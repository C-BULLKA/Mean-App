...
class PostController implements Controller {
    ...
    private initializeRoutes() {
        ...
        this.router.get('/:id/view', this.incrementView);
    ...
    private getAllPosts = async (request: Request, response: Response) => {
        try {
            ...
            const result = await this.dataService.getAllPaginated(page, limit);
    ...
    private incrementView = async (request: Request, response: Response) => {
        const { id } = request.params;
        try {
            const post = await this.dataService.incrementViews(id);
            if(post) response.status(200).json(post);
            else response.status(404).json({message: 'Not found'});
        } catch (error) {
            response.status(500).json({ error: 'Server error' });
